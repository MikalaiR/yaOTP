      ...
      pageBuilder: (
        ...
        animation.addStatusListener((status) {
          ...
          if (status == AnimationStatus.completed) {
            ...
            _showBiometricFirstController.close();
          }
        });
...
class LockScreen extends StatefulWidget {
  ...
  final FutureOr<bool> Function(BuildContext, String) onValidate;
...
class _LockScreenState extends State<LockScreen> {
  ...
  void _verifyCorrectString(String enteredValue) {
    Future.delayed(Duration(milliseconds: 150), () async {
      ...
      final validate = (context, pin) async {
        if (widget.onValidate is Future Function(BuildContext, String)) {
          return await widget.onValidate(context, pin);
        } else {
          return widget.onValidate(context, pin);
        }
      };

      if ((widget.onValidate != null &&
              await validate(context, enteredValue)) ||
          enteredValue == _verifyPasscode) {
  ...
  Widget _biometricButton() {
    ...
    return FlatButton(
      ...
      onPressed: () {
        if (widget.biometricAuthenticate == null) {
          throw Exception('specify biometricAuthenticate.');
        } else {
          widget.biometricAuthenticate(context).then((unlocked) {
            if (unlocked) {
              if (widget.onUnlocked != null) {
